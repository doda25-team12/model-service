...
def Generate_balanced_dataset(messages):
    ...
    balanced_data = (
        pd.concat([ham_msg, spam_msg]).sample(frac=1).reset_index(drop=True)
    )  # concatenate spam and ham messages to create a more balanced dataset
...
def main():
    ...
    X_train, X_test, y_train, y_test = train_test_split(
        feat_vect, balanced_data["label"], test_size=0.3, random_state=101
    )
